...
enum Story { //we'll give some of these associated values so we can pass in the user's name:
...
extension Story {
    var artwork: UIImage {
        return UIImage(named: self.rawValue)! //we can force unwrap becuase the image name and enum value will always match
    }
...
extension Page {
    ...
    func addChoice(title: String, page: Page) -> Page {
        ...
        case (.None, .None), (.None, .Some): //this way it will store it to firstChoice in either of these situations
...
struct Adventure {
    static func story(name: String) -> Page {
        ...
        let rover = touchdown.addChoice("Explor the Rover", story: .Rover(name))
